$(document).ready(function(){
	...
	$(document).on('click','#btn-cadastro',function(){
		if(validateEach() && isEmail($('input[name=email]').val())){
			...
			$.ajax({
				url: 'app/switch.php?method=cadastraUsuario',
				method:'POST',
	...
	$(document).on('click','#btn-login',function(){
		if(validateEach() && isEmail($('input[name=email]').val())){
			...
			$.ajax({
				url: 'app/switch.php?method=validaLogin',
				method: 'POST',
	...
	$(document).on('click','#btn-asks-reset',function(){
		...
		if(isEmail(email)){
			$.ajax({
				url: 'app/switch.php?method=asksResetPassword',
				data: {'email':email},
				beforeSend: function(){
					$('#email').val('');
	...
	$(document).on('click','#btn-reset',function(){
		...
		if(password == repeat_password && repeat_password != '' && password != ''){
			$.ajax({
				url: 'app/switch.php?method=resetPassword',
				method: 'POST',
